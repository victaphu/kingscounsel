...
import { selectColor, selectCurrentPlayer, selectEatenPieces, selectGameWon, selectIsGameStarted, selectJoined, selectReplayMode, selectTimer } from "@/app/redux/gameSlice";
...
const Player: React.FC<PlayerProps> = (props: PlayerProps) => {
  ...
  return (<div className="w-full p-2 cursor-pointer">
    <PlayerList show={open} players={props.players} setShow={setOpen} />
    <div className="flex flex-row lg:gap-4 p-2">
      ...
      {!replayMode && <Countdown date={timer} renderer={renderCountdown} ref={e => setApi(e?.api)} autoStart={false} now={() => Date.now()} />}
